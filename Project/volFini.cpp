...
// Finite Volume Main Function
void volFini(int n,int itermax)
{
  // Mesh Generating [-2,2]
  double xg=-2;
  double xd=2;
  ...
  double deltax=L/n;
  unsigned int ne = n+1; // Number of Elements

  // Mesh Elements
  Vector x(n+1,0.0);
  for (unsigned int k=0;k<ne;k++) x[k]=xg+k*deltax;
  ...
  Vector dt(itermax, 0.0);
  
  dt[0] = min_time;
  //Vector h0(n+1);
  //for (unsigned int j=0;j<ne;j++) if (x[j]>0) h0[j]=1; else h0[j]=2;

  FullMtx u(n_elements_time,ne,0.0);
  for (unsigned int k=0;k<ne;++k)
  if (center[k]>0) u[0][k]=1;
  else u[0][k]=2;
